...
{
	if(HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, _get_canrx_queue_header(), _get_canrx_queue_buf()) == HAL_OK)
	{
		if(_push_canrx_queue())
		{
			error_assert(ERR_FULLBUF_CANRX);
		}
	}
}
...
{
	if(HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO1, _get_canrx_queue_header(), _get_canrx_queue_buf()) == HAL_OK)
	{
		if(_push_canrx_queue())
		{
			error_assert(ERR_FULLBUF_CANRX);
